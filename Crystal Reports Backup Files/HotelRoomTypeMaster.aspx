<%@ Page Title="" Language="C#" MasterPageFile="~/Views/Shared/CRMMaster.Master" AutoEventWireup="true" CodeBehind="HotelRoomTypeMaster.aspx.cs" Inherits="CRM.WebApp.Views.Administration.HotelRoomTypeMaster" %>
...
<style>
    ...
    <telerik:radcodeblock id="RadCodeBlock1" runat="server">
            <script type="text/javascript" src="../Shared/Javascripts/HotelRoom.js"></script>
        <script type="text/javascript">
            function pageLoad() {

                HotelRoomTypeTableView = $find("<%= radgridhotelroomtype.ClientID %>").get_masterTableView();
                HotelRoomTypeCommand = "Load";
            ...
            function deleteCurrent() {
                var table = $find("<%= radgridhotelroomtype.ClientID %>").get_masterTableView().get_element();
                var row = table.rows[currentRowIndex]; table.deleteRow(currentRowIndex);
                var dataItem = $find(row.id);
                if (dataItem) {
                    dataItem.dispose();
                    Array.remove($find("<%= radgridhotelroomtype.ClientID %>").get_masterTableView()._dataItems, dataItem);
                }
                var gridItems = $find("<%= radgridhotelroomtype.ClientID %>").get_masterTableView().get_dataItems();
                CRM.WebApp.webservice.RoomTypeMasterWebService.deleteRoomType(ROOM_TYPE_ID);
                gridItems[gridItems.length - 1].set_selected(true);
            }
            function addnewhotelroomtype(sender, args) {
                ...
                try {
                    CRM.WebApp.webservice.RoomTypeMasterWebService.InsertUpdateRoom(ary);
    ...
    <div class="pageTitle" style="float: left">
        <asp:Literal ID="lblPageSite" runat="server" Text="Hotel Room Type Master"></asp:Literal>
    </div>
    <br />
    <br />
    ...
    <script type="text/javascript">
            ...
            <tr>
                <td>
                        <asp:Button ID="Delete" CssClass="button" Style="float: right; margin-right: 10px;
                            color: black; font-weight: bold;" OnClientClick="if(!confirm('Are you sure you want to delete this Room Type?'))return false; deleteCurrent(); return false;"
        ...
        <table>
            <tr>
               ...
               <MasterTableView ClientDataKeyNames="ROOM_TYPE_ID" AllowMultiColumSorting="true" EditMode ="InPlace" Width="400px">
               <RowIndicatorColumn>
               </RowIndicatorColumn>
                    <Columns>
                    ...
                    <telerik:GridTemplateColumn>
                     <ItemStyle CssClass="ItemAlign" Width="25px" />
                      <HeaderStyle Width="25px" />
                        <ItemTemplate>
                        <a id="A1" href="#" style="border:0px solid #fff;text-decoration:none;height:14px;width:14px;" onkeydown="addnewhotelroomtype(this,event);">
                            &raquo;
                            </a>
                        </ItemTemplate>
                    </telerik:GridTemplateColumn>
            ...
            <Resizing AllowColumnResize="true" ResizeGridOnColumnResize="true" />
                <ClientEvents OnCommand="radgridhotelroomtype_Command" OnRowSelected="radgridhotelroomtype_RowSelected"/>
