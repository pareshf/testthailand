        ...
        <script type="text/javascript" src="../Shared/Javascripts/SupplierGuidePrice.js"></script>
         ...
         <script type="text/javascript">
             ...
             function delCustomer() {
                 var table = $find("<%= radgridsupplierGuidePrice.ClientID %>").get_masterTableView().get_element();
                 var row = table.rows[currentRowIndex]; table.deleteRow(currentRowIndex);
                 var dataItem = $find(row.id);
                 if (dataItem) {
                     dataItem.dispose();
                     Array.remove($find("<%= radgridsupplierGuidePrice.ClientID %>").get_masterTableView()._dataItems, dataItem);
                 }
                 var gridItems = $find("<%= radgridsupplierGuidePrice.ClientID %>").get_masterTableView().get_dataItems();
             ...
             function addewGuidePrice(sender, args) {
                 ...
                 if (ary[10] != 0 && ary[11] != 0) {
                     ...
                     try {
                         CRM.WebApp.webservice.SupplierGuidePriceListWebService.InsertUpdateGuidePrice(ary);
                         CRM.WebApp.webservice.SupplierGuidePriceListWebService.GetGuidePrice(GuidePriceTableView.get_currentPageIndex() * GuidePriceTableView.get_pageSize(), GuidePriceTableView.get_pageSize(), GuidePriceTableView.get_sortExpressions().toString(), GuidePriceTableView.get_filterExpressions().toDynamicLinq(), sfname, scity, updateGuidePrice);
        ...
        <tr>
            <td>
                <telerik:radgrid id="radgridsupplierGuidePrice" runat="server" allowpaging="true"
                    allowmultirowselection="false" allowsorting="True" pagesize="25" itemstyle-wrap="false"
